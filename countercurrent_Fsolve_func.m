function[xc, yc, percentage_removed, E, R]= countercurrent_Fsolve_func(S,F,Stages)
...
%     B1=B1(400:end);
    ...
    while(i<Stages)
     % intersection of the raffinate point of previous stage to delta with the equilibrium curve giving Extract point of current stage %%%%%
        ...
        e0=[0.6 1];
        ye = fzero(@(x)extract_function(x,slope_R(i),rx,ry,p1), e0);
        ex=ye;ey=slope_R(i)*(ye-rx)+ry;
      ...
      % finding the raffinate %
        ...
        r0=[0 0.2];
        xr = fzero(@(x)raffinate_function(x,slope_1(i),ex,ey,p2), r0);
        xbr=xr;xcr=slope_1(i)*(xr-ex)+ey;
 ...
 %%
    ...
    % finding the final extract and raffinate flowrates
    syms Ei Ri
     [E(1), R(i)]=vpasolve([F+S==Ei+Ri,Ey(1)*Ei+Ri*Ry(i)==sol_tot],[Ei,Ri],[0 Inf;0 Inf]);
    
    
    
    for j=1:i-1
        [E(j+1), R(j)]=vpasolve([F-E(1)==-Ei+Ri,F*xcf-E(1)*Ey(1)==-Ei*Ey(j+1)+Ri*Ry(j)],[Ei,Ri],[0 F+S;0 F+S]);
    end    
    
    
    extracted=zeros(i,1); % percentage extracted over each stage
    for j=1:i
    
        extracted(j)=E(i-j+1)*Ey(i-j+1)*100/(sol_tot);
    
    end    
    percentage_removed=extracted;
    yc=Ry;
    xc=Ey;
end
